...
const SERVER_TOKEN = process.env.BLOB_READ_WRITE_TOKEN || process.env.VERCEL_BLOB_TOKEN;

export async function POST(req: NextRequest) {
  try {
    if (!SERVER_TOKEN) {
      return NextResponse.json({ error: "Vercel Blob token not configured (BLOB_READ_WRITE_TOKEN)" }, { status: 500 });
    }
    ...
    const res = await vercelPut(key, { token: SERVER_TOKEN, contentType });

    // The SDK returns an object with url/downloadUrl/pathname; it may also include an uploadUrl for PUT
    const uploadUrl = (res as any).uploadUrl || (res as any).uploadURL || (res as any).url;
    const publicUrl = (res as any).url || (res as any).downloadUrl || `https://cdn.vercel.com/${key}`;
